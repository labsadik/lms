...
-- Bypasses RLS securely so normal users can see each other's total XP/Coins
-- for the leaderboard UI without exposing raw coin_ledger rows.
-- ---------------------------------------------------------------------
...
-- SYSTEM BADGE INSERT POLICY: Allows the trigger to automatically award badges safely
...
BEGIN
  -- Only check when a video minute is logged
  IF NEW.source = 'video' THEN
    
    -- Count how many video minutes this user has total
    SELECT COUNT(*) INTO v_video_count 
    FROM public.coin_ledger 
    WHERE user_id = NEW.user_id AND source = 'video';
    
    -- If this is their FIRST EVER minute, award the badge!
